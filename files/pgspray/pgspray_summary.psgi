...
my $summa;
$summa->{$_} = $prom->{$_}->new_summary ( name    => $metric_name,
		...
		alarm 0; # cancel the alarm 
		        ...
		        for (@clusters) {
			        ...
			        for my $quantile (qw/ 0.5  0.9  0.99/  ) {
			                $msg->{$_} .=sprintf qq(%s{hostname="%s",host_ip="%s",pg_port="%d",cluster="%s",quantile="%s"} %4f\n), 
			                $metric_name, $conf->{$_}{hostname},$conf->{$_}{host_ip}, $conf->{$_}{port} ,
			   	        $conf->{$_}{cluster}, $quantile,  calc_percentile( $quantile*100 ,$res->{$_});
			        } # for quantile
                                $msg->{$_} =  $prom->{$_}->render . $msg->{$_} ;
                        } # for clusters    
say Dumper $msg;
                return [
                        '200',
